...
var x = {
...
).replace("comment", je).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(), I = /(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/, Qe = u(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label", I).replace("href", /<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(), fe = u(/^!?\[(label)\]\[(ref)\]/).replace("label", I).replace("ref", H).getRegex(), ke = u(/^!?\[(ref)\](?:\[\])?/).replace("ref", H).getRegex(), Fe = u("reflink|nolink(?!\\()", "g").replace("reflink", fe).replace("nolink", ke).getRegex(), X = {
  _backpedal: z,
  ...
  ...X,
  link: u(/^!?\[(label)\]\((.*?)\)/).replace("label", I).getRegex(),
  reflink: u(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", I).getRegex()
}, D = {
  ...X,
...
`, 1)[0];
            let T;
            if (p = Z, this.options.pedantic ? (p = p.replace(this.rules.other.listReplaceNesting, "  "), T = p) : T = p.replace(this.rules.other.tabCharGlobal, "    "), V.test(p) || Y.test(p) || de.test(p) || q.test(p) || K.test(p))
              break;
            if (T.search(this.rules.other.nonSpaceChar) >= f || !p.trim())
              h += `
` + T.slice(f);
            else {
...
` + p;
            ...
            !d && !p.trim() && (d = !0), o += Z + `
`, n = n.substring(Z.length + 1), k = T.slice(f);
          }
        }
...
` ? e[1].slice(0, -1) : e[1];
  ...
  inlineQueue;
  constructor(e) {
    this.tokens = [], this.tokens.links = /* @__PURE__ */ Object.create(null), this.options = e || S, this.options.tokenizer = this.options.tokenizer || new P(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
      inLink: !1,
      inRawBlock: !1,
      top: !0
    };
    ...
    this.options.pedantic ? (t.block = A.pedantic, t.inline = $.pedantic) : this.options.gfm && (t.block = A.gfm, this.options.breaks ? t.inline = $.breaks : t.inline = $.gfm), this.tokenizer.rules = t;
  }
  /**
   * Expose Rules
   */
...
`;
  ...
  textRenderer;
  constructor(e) {
    this.options = e || S, this.options.renderer = this.options.renderer || new C(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new W();
  }
  /**
   * Static Parse Method
   */
  ...
  use(...n) {
    ...
    return n.forEach((t) => {
      ...
      if (s.async = this.defaults.async || s.async || !1, t.extensions && (t.extensions.forEach((r) => {
        ...
        const r = this.defaults.renderer || new C(this.defaults);
...
g.Hooks = L;
g.parse = g;
var Ye = g.options, et = g.setOptions, tt = g.use, nt = g.walkTokens, rt = g.parseInline, st = g, it = y.parse, lt = w.lex;
export {
  L as Hooks,
  w as Lexer,
  Ve as Marked,
