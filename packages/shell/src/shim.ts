...
const showChrome = root.dataset.chrome !== 'hidden'

let input: HTMLInputElement | null = null
let loginBtn: HTMLButtonElement | null = null
let logoutBtn: HTMLButtonElement | null = null
let userInfo: HTMLSpanElement | null = null

if (showChrome) {
  // Skip nav
  const skipNav = document.createElement('a')
  skipNav.href = '#pane-container'
  skipNav.className = 'skip-nav'
  skipNav.textContent = 'Skip to content'
  ...
  input.required = true
  input.setAttribute('aria-label', 'Resource URL')
...
async function init(): Promise<void> {
  ...
  if (input) input.value = initialUri
