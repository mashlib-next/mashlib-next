...
function getPrice(subject, store) {
  ...
  if (offerNode) {
    const offerPrice = store.any(offerNode, SCHEMA("price"), null, null)?.value;
    if (offerPrice) {
      const currency = store.any(offerNode, SCHEMA("priceCurrency"), null, null)?.value ?? "";
...
function renderProduct(subject, store, container) {
  container.innerHTML = "";
  ...
  if (brandNode) {
    const brandName = store.any(brandNode, SCHEMA("name"), null, null)?.value ?? brandNode.value;
  ...
  if (ratingNode) {
    const ratingValue = store.any(ratingNode, SCHEMA("ratingValue"), null, null)?.value;
    const reviewCount = store.any(ratingNode, SCHEMA("reviewCount"), null, null)?.value;
    const bestRating = store.any(ratingNode, SCHEMA("bestRating"), null, null)?.value ?? "5";
    if (ratingValue) {
      const ratingEl = document.createElement("div");
      ratingEl.className = "product-rating";
      const stars = Math.round(parseFloat(ratingValue));
      const maxStars = parseInt(bestRating);
      const starText = "\u2605".repeat(Math.min(stars, maxStars)) + "\u2606".repeat(Math.max(maxStars - stars, 0));
