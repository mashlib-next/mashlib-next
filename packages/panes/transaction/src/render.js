...
function isAccount(subject, store) {
...
function extractTransaction(node, store) {
  const description = store.any(node, QU("description"), null, null)?.value ?? store.any(node, DCT("title"), null, null)?.value ?? store.any(node, SCHEMA("description"), null, null)?.value ?? store.any(node, SCHEMA("name"), null, null)?.value ?? labelFromUri(node.value);
  const amountStr = store.any(node, QU("amount"), null, null)?.value ?? store.any(node, SCHEMA("price"), null, null)?.value ?? store.any(node, SCHEMA("amount"), null, null)?.value;
  const amount = amountStr ? parseFloat(amountStr) : null;
  const currency = store.any(node, QU("currency"), null, null)?.value ?? store.any(node, SCHEMA("priceCurrency"), null, null)?.value ?? store.any(node, SCHEMA("currency"), null, null)?.value ?? "";
  const date = store.any(node, QU("date"), null, null)?.value ?? store.any(node, DCT("date"), null, null)?.value ?? store.any(node, SCHEMA("dateCreated"), null, null)?.value ?? null;
  const categoryNode = store.any(node, QU("category"), null, null);
  const category = categoryNode ? store.any(categoryNode, RDFS("label"), null, null)?.value ?? store.any(categoryNode, SCHEMA("name"), null, null)?.value ?? labelFromUri(categoryNode.value) : null;
  const payeeNode = store.any(node, QU("payee"), null, null) ?? store.any(node, SCHEMA("recipient"), null, null);
  const payee = payeeNode ? store.any(payeeNode, SCHEMA("name"), null, null)?.value ?? (payeeNode.value.startsWith("http") ? labelFromUri(payeeNode.value) : payeeNode.value) : null;
  return { uri: node.value, date, description, amount, currency, category, payee };
}
function renderAccount(subject, store, wrapper) {
  const transactions = store.each(subject, QU("transaction"), null, null);
  const records = transactions.map((t) => extractTransaction(t, store));
  records.sort((a, b) => {
  ...
  for (const rec of records) {
    ...
    if (rec.date) {
      ...
      dateTd.textContent = isNaN(d.getTime()) ? rec.date : d.toLocaleDateString(void 0, { month: "short", day: "numeric" });
    }
...
function renderSingleTransaction(subject, store, wrapper) {
  ...
  if (rec.date) {
    const d = new Date(rec.date);
    const formatted = isNaN(d.getTime()) ? rec.date : d.toLocaleDateString(void 0, { year: "numeric", month: "short", day: "numeric" });
  ...
  const desc = store.any(subject, SCHEMA("description"), null, null)?.value ?? store.any(subject, DCT("description"), null, null)?.value;
...
function renderTransaction(subject, store, container) {
  container.innerHTML = "";
  const wrapper = document.createElement("div");
  wrapper.className = "txn-view";
  const title = store.any(subject, SCHEMA("name"), null, null)?.value ?? store.any(subject, DCT("title"), null, null)?.value ?? store.any(subject, QU("description"), null, null)?.value ?? store.any(subject, RDFS("label"), null, null)?.value ?? labelFromUri(subject.value);
