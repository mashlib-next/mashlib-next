...
/**
 * Render a code file into the container.
 */
export function renderCode(
  ...
  // Try to get content from the store first
  const content =
    store.any(subject, SOLID('content'), null, null)?.value ??
    store.any(subject, DCT('content'), null, null)?.value
  ...
  // Fallback: fetch the file content
 ...
 */
function renderCodeBlock(code: string, wrapper: HTMLElement): void {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Use innerHTML with escaped content to preserve whitespace
