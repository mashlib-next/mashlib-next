...
/**
 * Map AS2 activity type to a human-readable verb.
 */
...
/**
 * Collect notifications from an inbox.
 */
function collectNotifications(subject: NamedNode, store: Store): NotificationItem[] {
  ...
  for (const node of contained) {
    ...
    if (actorNode) {
      ...
      actorName =
        store.any(actorNode as NamedNode, SCHEMA('name'), null, null)?.value ??
        store.any(actorNode as NamedNode, AS('name'), null, null)?.value ??
        labelFromUri(actorNode.value)
    }
    ...
    if (objectNode) {
      ...
      objectName =
        store.any(objectNode as NamedNode, SCHEMA('name'), null, null)?.value ??
        store.any(objectNode as NamedNode, AS('name'), null, null)?.value ??
        labelFromUri(objectNode.value)
    }

    // Content / summary
    const content =
      store.any(n, AS('content'), null, null)?.value ??
      store.any(n, AS('summary'), null, null)?.value ??
      store.any(n, DCT('description'), null, null)?.value ??
      null

    // Timestamp
    const timestamp =
      store.any(n, AS('published'), null, null)?.value ??
      store.any(n, DCT('created'), null, null)?.value ??
      store.any(n, DCT('date'), null, null)?.value ??
      null
...
export function renderNotifications(
  ...
  for (const item of items) {
    ...
    // Summary line: Actor verbed Object
