...
function getParticipantName(node, store) {
  return store.any(node, SCHEMA("name"), null, null)?.value ?? store.any(node, FOAF("name"), null, null)?.value ?? labelFromUri(node.value);
}
function renderMeeting(subject, store, container) {
  container.innerHTML = "";
  ...
  const dateStr = store.any(subject, SCHEMA("startDate"), null, null)?.value ?? store.any(subject, DCT("created"), null, null)?.value ?? store.any(subject, DCT("date"), null, null)?.value;
  if (dateStr) {
  ...
  const location = store.any(subject, SCHEMA("location"), null, null)?.value ?? store.any(subject, MEE("location"), null, null)?.value;
  ...
  if (agendaItems.length > 0) {
    ...
    for (const item of agendaItems) {
      const li = document.createElement("li");
      const itemTitle = store.any(item, DCT("title"), null, null)?.value ?? store.any(item, SCHEMA("name"), null, null)?.value ?? item.value;
  ...
  if (actions.length > 0) {
    ...
    for (const action of actions) {
      const li = document.createElement("li");
      const actionText = store.any(action, DCT("title"), null, null)?.value ?? store.any(action, SCHEMA("name"), null, null)?.value ?? store.any(action, RDFS("label"), null, null)?.value ?? action.value;
  ...
  const notes = store.any(subject, SIOC("content"), null, null)?.value ?? store.any(subject, SCHEMA("text"), null, null)?.value;
  if (notes) {
    const section = document.createElement("div");
    section.className = "meeting-section";
    const heading = document.createElement("h3");
