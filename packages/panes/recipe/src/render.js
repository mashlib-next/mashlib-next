...
function renderRecipe(subject, store, container) {
  container.innerHTML = "";
  ...
  if (ratingNode) {
    ...
    if (ratingValue) {
      ...
      const starText = "\u2605".repeat(Math.min(stars, 5)) + "\u2606".repeat(Math.max(5 - stars, 0));
  ...
  if (instructions.length > 0) {
    ...
    for (const step of instructions) {
      const stepText = store.any(step, SCHEMA("text"), null, null)?.value ?? step.value;
  ...
  if (nutritionNode) {
    ...
    for (const [prop, label] of nutritionProps) {
      const val = store.any(nutritionNode, SCHEMA(prop), null, null)?.value;
  ...
  if (authorNode) {
    const authorName = store.any(authorNode, SCHEMA("name"), null, null)?.value ?? authorNode.value;
