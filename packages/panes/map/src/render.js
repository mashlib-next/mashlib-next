...
function renderMap(subject, store, container) {
  container.innerHTML = "";
  ...
  const address = store.any(subject, SCHEMA("address"), null, null)?.value ?? void 0;
  if (address) {
  ...
  if (coords) {
    ...
    iframe.src = `https://www.openstreetmap.org/export/embed.html?bbox=${coords.lng - 0.01},${coords.lat - 5e-3},${coords.lng + 0.01},${coords.lat + 5e-3}&layer=mapnik&marker=${coords.lat},${coords.lng}`;
